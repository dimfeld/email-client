...
const configuredAccount = listAccounts(getDatabase()).find(
  (candidate) => candidate.email === account
);
if (!configuredAccount?.refreshToken)
  throw new Error(`Connect ${account} through Settings before importing mail.`);
const maxMessages = limit === undefined ? undefined : Number(limit);
if (maxMessages !== undefined && (!Number.isInteger(maxMessages) || maxMessages <= 0))
