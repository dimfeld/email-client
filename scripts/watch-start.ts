...
if (oauthFile) {
	...
	try {
		...
		throw new Error(`Could not read OAuth JSON file: ${error instanceof Error ? error.message : String(error)}`);
	}
	...
	if (!credentialsValue || typeof credentialsValue !== 'object' || Array.isArray(credentialsValue)) {
	...
	if ((clientId === undefined) !== (clientSecret === undefined)) {
		throw new Error('OAuth JSON file must contain both client_id and client_secret when either is provided.');
	}
	...
	if (!account.refreshToken) throw new Error(`Connect ${email} through Settings or provide --oauth-file before starting its watch.`);
	result = await googleApiRequest<{ historyId?: unknown }>(account,
