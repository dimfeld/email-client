...
import { createDatabase, deleteRemoteImageRule, listRemoteImageRules, saveRemoteImageRule } from './server/db';
...
describe('remote image sender rules', () => {
	it('matches an exact sender address or domain without matching a different domain', () => {
		...
		expect(allowsRemoteImages('News@Example.com', [{ kind: 'address', value: 'news@example.com' }])).toBe(true);
		expect(allowsRemoteImages('other@example.com', [{ kind: 'address', value: 'news@example.com' }])).toBe(false);
		expect(allowsRemoteImages('other@example.com', [{ kind: 'domain', value: 'example.com' }])).toBe(true);
		expect(allowsRemoteImages('other@sub.example.com', [{ kind: 'domain', value: 'example.com' }])).toBe(false);
		expect(allowsRemoteImages('other@notexample.com', [{ kind: 'domain', value: 'example.com' }])).toBe(false);
