...
export function viewRange(view: CalendarView, key: DateKey): DateRange {
  switch (view) {
    case 'day':
      return { start: key, end: addDays(key, 1) };
    case 'week': {
...
export function shiftView(view: CalendarView, key: DateKey, direction: -1 | 1): DateKey {
  switch (view) {
    case 'day':
      return addDays(key, direction);
    case 'week':
      return addDays(key, direction * 7);
    case 'month':
...
export function eventCalendarKey(
  event: Pick<SyncedCalendarEvent, 'accountEmail' | 'calendarId'>
): string {
...
export function eventInterval(
  event: Pick<SyncedCalendarEvent, 'startAt' | 'endAt' | 'allDay'>
): Interval {
...
export function layoutTimedEvents(events: SyncedCalendarEvent[], key: DateKey): TimedPlacement[] {
  ...
  const timed = events
    .filter((event) => !event.allDay && eventOccursOn(event, key))
    .sort(compareEvents);
  ...
  for (const event of timed) {
    ...
    const endMinutes = Math.min(
      dayMinutes,
      Math.max(startMinutes, (interval.end - day.start) / minute)
    );
    ...
    const placement: TimedPlacement = {
      ...
      column,
      columns: 1,
      continuesBefore: interval.start < day.start,
      continuesAfter: interval.end > day.end,
    };
