...
describe('email HTML documents', () => {
	it('keeps email head styles after the compatibility and security rules', () => {
		...
		expect(document.indexOf('Content-Security-Policy')).toBeLessThan(document.indexOf('.message{color:red}'));
	...
	it('detects remote image sources and CSS background images', () => {
		...
		expect(hasRemoteImages('<div style="background:url(http://example.com/image.png)">')).toBe(true);
