...
describe('configurable Jev categories', () => {
  it('sends category names and descriptions and reads changes on each call', async () => {
    ...
    try {
      ...
      expect(request.mock.calls[0][0].questions.category).toEqual(
        expect.objectContaining({ criteria: { custom: 'Travel: Flights and hotels.' } })
      );
      expect(request.mock.calls[0][0].questions.actionItem).toEqual(
        expect.objectContaining({
          type: 'noul',
      ...
      expect(request.mock.calls[0][0].questions.reminder).toEqual(
        expect.objectContaining({
          ...
          criteria: expect.objectContaining({
            true: expect.stringContaining('likely to track with a reminder'),
          }),
        })
      );
      categories = [
        { id: 'custom', name: 'Trips', description: 'Upcoming trips only.', level: 'other' },
      ];
      await classify({ id: 'message-2' });
      expect(request.mock.calls[1][0].questions.category).toEqual(
        expect.objectContaining({ criteria: { custom: 'Trips: Upcoming trips only.' } })
      );
      categories = [];
...
for (const importance of ['important', 'useful', 'other'] as const) {
  it(`stores the three-state importance result for Auto categories: ${importance}`, async () => {
    ...
    try {
      ...
      expect(Object.keys(request.mock.calls[0][0].questions.importance.criteria ?? {})).toEqual([
        'important',
        'useful',
        'other',
      ]);
