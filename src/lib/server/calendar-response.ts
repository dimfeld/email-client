...
export async function respondToCalendarInvite(
  ...
  if (!stored || !canRespondToEvent(stored))
    throw new Error('This event is not an invitation on your own calendar.');
  ...
  try {
    ...
    const attendee = event.attendees?.find(
      (item) =>
        item.self && item.email?.toLowerCase() === account.email.toLowerCase() && !item.organizer
    );
    if (!attendee) throw new Error('The connected account is not the invited attendee.');
    if (!event.etag)
      throw new Error(
        'Google did not return an event version. Refresh the calendar and try again.'
      );
    ...
    const updated = await request<RemoteEvent>(account, url, {
      ...
      data: {
        ...
        attendees: [{ email: attendee.email, responseStatus: response }],
      },
    });
    const actual = updated.attendees?.find(
      (item) => item.email?.toLowerCase() === account.email.toLowerCase()
    )?.responseStatus;
    if (!actual)
      throw new Error(
        'Google did not confirm your response. Refresh the calendar to check its status.'
      );
    saveCalendarResponse(database, account.email, calendarId, eventId, actual);
    if (actual !== response)
      throw new Error(
        `Google returned the response ${actual}. Refresh the calendar before trying again.`
      );
    return actual;
  } catch (error) {
    if (error instanceof GoogleApiError && error.status === 403)
      throw new Error(
        'Calendar write access is required. Reconnect this account in Settings and grant Calendar access, then try again.'
      );
    if (error instanceof GoogleApiError && error.status === 412)
      throw new Error(
        'The event changed while you replied. Refresh the calendar and review it before trying again.'
      );
    throw error;
  }
}
