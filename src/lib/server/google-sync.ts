...
export function normalizeContact(
...
): Omit<SyncedContact, 'accountEmail'> {
  ...
  const names = Array.isArray(contact.names)
    ? (contact.names as Array<Record<string, unknown>>)
    : [];
  const organizations = Array.isArray(contact.organizations)
    ? (contact.organizations as Array<Record<string, unknown>>)
    : [];
  const organizationName =
    typeof organizations[0]?.name === 'string' ? organizations[0].name : undefined;
  const organizationTitle =
    typeof organizations[0]?.title === 'string' ? organizations[0].title : undefined;
...
): Omit<SyncedCalendar, 'accountEmail'> {
...
export function normalizeCalendarEvent(
  ...
  calendarId: string
): Omit<SyncedCalendarEvent, 'accountEmail'> {
  const start = eventTime(event.start);
  const end = eventTime(event.end);
  if (typeof event.id !== 'string' || !start || !end)
    throw new Error('A Google calendar event did not include its id, start, or end.');
  return {
    calendarId,
    eventId: event.id,
    ...
    responseStatus:
      (Array.isArray(event.attendees) ? event.attendees : []).find((item) => item.self === true)
        ?.responseStatus ?? null,
...
type OtherContactsPage = {
  otherContacts?: Record<string, unknown>[];
  nextPageToken?: string;
  nextSyncToken?: string;
};
type CalendarPage = {
...
function isDeleted(value: Record<string, unknown>): boolean {
  return (
    ...
    Boolean(
      ...
      (value.metadata as Record<string, unknown>).deleted
    )
  );
}
...
function isExpiredSyncToken(error: unknown): boolean {
  return (
    (error instanceof Error &&
      /expired[^.]*sync[^.]*token|sync[^.]*token[^.]*expired/i.test(error.message)) ||
...
async function syncGoogleAccountOnce(
  ...
  const fullSync = async (): Promise<GoogleSyncResult> => {
      ...
      while (otherContactsProgress.phase !== 'complete') {
        ...
        saveGoogleOtherContactsSyncPage(
          ...
          (result.otherContacts ?? [])
            .filter((contact) => !isDeleted(contact))
    ...
    if (!deferred) {
      ...
      while (calendarProgress.phase === 'calendarList') {
        ...
        const next = saveCalendarListSyncPage(
          database,
          calendarProgress,
      ...
      while (!deferred && calendarProgress.phase === 'calendarEvents') {
        ...
        const next = saveCalendarEventsSyncPage(
          database,
          calendarProgress,
          (result.items ?? [])
            .filter((event) => !isDeleted(event) && event.status !== 'cancelled')
    ...
    if (!deferred) {
      ...
      if (
        contactsProgress?.phase === 'complete' &&
        otherContactsProgress?.phase === 'complete' &&
        calendarProgress?.phase === 'complete'
      )
        finalizeGoogleSync(database, account.email);
    }
  ...
  const calendarProgress = getGoogleSyncProgress(database, account.email, 'calendar');
  if (
    (contactsProgress?.phase === 'complete' && !contactsProgress.nextSyncToken) ||
  ...
  if (
    !state.contactsSyncToken ||
    !state.otherContactsSyncToken ||
    !state.calendarListSyncToken ||
    contactsProgress ||
    otherContactsProgress ||
    calendarProgress
  ) {
  ...
  try {
    ...
    do {
      const result = await requestPage<OtherContactsPage>(
        'https://people.googleapis.com/v1/otherContacts',
        {
          ...
          pageToken: otherContactsPageToken,
        }
      );
      fetched.contacts += result.otherContacts?.length ?? 0;
      for (const contact of result.otherContacts ?? []) {
        ...
        if (isDeleted(contact)) {
          if (typeof contact.resourceName === 'string')
            deletedOtherContacts.add(contact.resourceName);
    ...
    applyGoogleOtherContactsIncrementalSync(
      database,
      account.email,
      changedOtherContacts,
      [...deletedOtherContacts],
    ...
    do {
      const result = await requestPage<CalendarPage>(
        'https://www.googleapis.com/calendar/v3/users/me/calendarList',
        {
          showDeleted: true,
          showHidden: true,
          syncToken: state.calendarListSyncToken,
    ...
    for (const [calendarId, savedSyncToken] of getGoogleSyncState(database, account.email)
      .calendarEventSyncTokens) {
      ...
      applyCalendarEventsIncrementalSync(
        ...
        changedEvents,
        deletedEvents,
        requireSyncToken(eventNextSyncToken, `Google Calendar ${calendarId}`),
        replace
      );
    }
    return fetched;
...
export async function syncConfiguredGoogleAccounts(
  ...
  const accounts = listAccounts(database).filter(
    (account) =>
      account.enabled && account.refreshToken && (!accountEmail || account.email === accountEmail)
  );
  if (accountEmail && accounts.length === 0)
    throw new Error(`No connected account is configured for ${accountEmail}.`);
  const results = [];
  for (const account of accounts)
    results.push({
      account: account.email,
      result: await syncGoogleAccount(database, account, request, options),
    });
  return results;
}
