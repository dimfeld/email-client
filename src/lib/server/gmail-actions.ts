...
export async function runGmailMessageAction(account: GoogleAccount, gmailId: string, action: GmailMessageAction, request: typeof googleApiRequest = googleApiRequest): Promise<void> {
	...
	await request(account, action === 'archive' ? `${messageUrl}/modify` : `${messageUrl}/trash`, {
		method: 'POST', data: action === 'archive' ? { removeLabelIds: ['INBOX'] } : undefined
	});
}

export async function applyGmailMessageAction(database: DatabaseSync, account: GoogleAccount, gmailId: string, action: GmailMessageAction, request: typeof googleApiRequest = googleApiRequest): Promise<void> {
