...
export async function syncGoogleData({
  ...
  GoogleDataSyncDependencies,
  'database' | 'request' | 'syncOptions'
>): Promise<GoogleDataSyncResult> {
...
export function startGoogleDataSync({
  ...
  if (!listAccounts(database).some((account) => account.enabled && account.refreshToken))
    return { close() {} };
  let running = false;
  const run = async () => {
    if (running) return;
