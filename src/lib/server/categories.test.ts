...
import { createDatabase, deleteCategory, listCategories, listEmails, saveCategory, saveClassification, upsertEmails } from './db';
...
describe('category settings', () => {
	it('migrates saved importance flags to levels without losing category settings', () => {
		...
		saveCategory(database, { id: 'custom', name: 'Travel', description: 'Bookings.', level: 'useful' });
	...
	it('migrates stored useful and not-useful messages to the new importance values', () => {
		...
		const path = join(directory, 'test.sqlite');
		database = createDatabase(path);
		upsertEmails(database, 'test@example.com', [{ id: 'useful' }, { id: 'other' }, { id: 'pending' }]);
		...
		database.close();
		database = createDatabase(path);
		expect(Object.fromEntries(listEmails(database).map((email) => [email.gmailId, email.importance])))
	...
	it('seeds the existing category IDs with Action needed important and preserves edits on restart', () => {
		...
		expect(categories.map((category) => category.id)).toEqual(['action', 'personal', 'work', 'transaction', 'newsletter', 'notification', 'marketing', 'other']);
		expect(categories.filter((category) => category.level === 'important').map((category) => category.id)).toEqual(['action']);
		saveCategory(database, { ...categories[0], name: 'Follow up', description: 'Messages that need a reply.', level: 'other' });
		deleteCategory(database, 'marketing');
		database.close();
		database = createDatabase(path);
		expect(listCategories(database)[0]).toEqual({ id: 'action', name: 'Follow up', description: 'Messages that need a reply.', level: 'other' });
	...
	it('keeps message membership when a category is renamed or marked important', () => {
		...
		saveCategory(database, { id: 'action', name: 'Reply', description: 'Reply to this message.', level: 'important' });
		const [email] = listEmails(database);
		expect(email.category).toBe('action');
		expect(email.importance).toBe('other');
		expect(listCategories(database).find((category) => category.id === email.category)?.level).toBe('important');
	});

	it('adds custom categories and rejects blank or duplicate names without changing saved settings', () => {
		database = createDatabase(':memory:');
		const id = saveCategory(database!, { name: ' Travel ', description: ' Travel plans. ', level: 'important' });
		expect(listCategories(database).find((category) => category.id === id)).toEqual({ id, name: 'Travel', description: 'Travel plans.', level: 'important' });
		expect(() => saveCategory(database!, { name: 'travel', description: 'Duplicate', level: 'other' })).toThrow('already exists');
		expect(() => saveCategory(database!, { id, name: ' ', description: 'Test', level: 'other' })).toThrow('name and a description');
		expect(() => saveCategory(database!, { id, name: 'Travel', description: ' ', level: 'other' })).toThrow('name and a description');
		expect(listCategories(database).find((category) => category.id === id)?.level).toBe('important');
	});
	...
	it('moves messages from a removed category to pending and rejects results for a removed category', () => {
		...
		expect(() => saveClassification(database!, 'test@example.com', message.id, classification)).toThrow('no longer exists');
	});
});
