...
describe('OpenAI email extraction', () => {
  ...
  it('retries once on the regular tier after a Flex resource error', async () => {
    ...
    const extract = createOpenAIEmailExtractor(
      'test-key',
      generateMock as unknown as typeof generateObject
    );

    await extract!(
      { id: 'message', subject: 'Plan', bodyText: 'The plan starts September 25.' },
      {
        actionItems: false,
        reminders: true,
      }
    ...
    expect(
      (requests[0].providerOptions as { openai: { serviceTier: string } }).openai.serviceTier
    ).toBe('flex');
    expect(
