...
CREATE INDEX IF NOT EXISTS idx_other_contacts_name
ON other_contacts(display_name COLLATE NOCASE);
...
CREATE TABLE IF NOT EXISTS google_sync_other_contacts (
  account_email TEXT NOT NULL,
  sync_id TEXT NOT NULL,
  resource_name TEXT NOT NULL,
  display_name TEXT NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS google_other_contacts_sync_progress (
...
export function createDatabase(path = defaultPath): DatabaseSync {
  ...
  if (!accountColumns.some((column) => column.name === 'other_contacts_sync_token')) {
    database.exec('ALTER TABLE accounts ADD COLUMN other_contacts_sync_token TEXT');
  }
  ...
  const progressColumns = database
    .prepare('PRAGMA table_info(google_sync_progress)')
  ...
  const stagedCalendarColumns = database
    .prepare('PRAGMA table_info(google_sync_calendars)')
    .all() as Array<{ name: string }>;
  ...
  withTransaction(database, () => {
    ...
    if (!exists) {
      ...
      for (const category of defaultCategories)
        insert.run(category.id, category.name, category.description, category.level);
    } else {
      const columns = database.prepare('PRAGMA table_info(categories)').all() as Array<{
        name: string;
      }>;
      const table = database
        .prepare("SELECT sql FROM sqlite_master WHERE name = 'categories'")
        .get() as { sql: string };
    ...
    const emailColumns = database.prepare('PRAGMA table_info(emails)').all() as Array<{
      name: string;
    }>;
    ...
    database.exec(
      "UPDATE emails SET body_text = '' WHERE body_html IS NOT NULL AND body_text = body_html"
    );
...
export function listRemoteImageRules(database: DatabaseSync): RemoteImageRule[] {
  return database
    .prepare('SELECT kind, value FROM remote_image_rules ORDER BY kind, value')
    .all() as RemoteImageRule[];
}

export function saveRemoteImageRule(database: DatabaseSync, rule: RemoteImageRule): void {
  database
    .prepare('INSERT OR IGNORE INTO remote_image_rules (kind, value) VALUES (?, ?)')
...
export type GoogleOtherContactsSyncProgress = {
  accountEmail: string;
  syncId: string;
  phase: string;
  pageToken: string | null;
...
export function getGoogleSyncProgress(
  ...
  const row = database
    .prepare('SELECT * FROM google_sync_progress WHERE account_email = ? AND sync_type = ?')
    .get(accountEmail, syncType) as Record<string, unknown> | null;
  return row ? readGoogleSyncProgress(row) : null;
}

function readGoogleOtherContactsSyncProgress(
  row: Record<string, unknown>
): GoogleOtherContactsSyncProgress {
  return {
    accountEmail: String(row.account_email),
    syncId: String(row.sync_id),
    phase: String(row.phase),
...
export function getGoogleOtherContactsSyncProgress(
  database: DatabaseSync,
  accountEmail: string
): GoogleOtherContactsSyncProgress | null {
  const row = database
    .prepare('SELECT * FROM google_other_contacts_sync_progress WHERE account_email = ?')
    .get(accountEmail) as Record<string, unknown> | null;
  return row ? readGoogleOtherContactsSyncProgress(row) : null;
}

export function startGoogleOtherContactsSync(
  database: DatabaseSync,
  accountEmail: string
): GoogleOtherContactsSyncProgress {
  const existing = getGoogleOtherContactsSyncProgress(database, accountEmail);
  ...
  withTransaction(database, () => {
    database
      ...
      .run(accountEmail);
    database
      .prepare(`INSERT INTO google_other_contacts_sync_progress
		(account_email, sync_id, phase, page_token, next_sync_token, started_at, updated_at)
		VALUES (?, ?, 'otherContacts', NULL, NULL, ?, ?)`)
      .run(accountEmail, syncId, startedAt, startedAt);
  });
  return {
    accountEmail,
    syncId,
...
export function startGoogleSync(
  ...
  withTransaction(database, () => {
    if (syncType === 'contacts') {
      ...
      database
        .prepare('DELETE FROM google_sync_calendars WHERE account_email = ?')
        .run(accountEmail);
      database
        .prepare('DELETE FROM google_sync_calendar_events WHERE account_email = ?')
        .run(accountEmail);
    }
    database
      .prepare(`INSERT INTO google_sync_progress
...
function countRows(
  ...
  const row = syncId
    ? database
        .prepare(`SELECT COUNT(*) AS count FROM ${table} WHERE account_email = ? AND sync_id = ?`)
        .get(accountEmail, syncId)
    : database
        .prepare(`SELECT COUNT(*) AS count FROM ${table} WHERE account_email = ?`)
        .get(accountEmail);
  return Number((row as { count: number }).count);
}

export function getGoogleSyncCounts(
  database: DatabaseSync,
  accountEmail: string
): {
  contacts: number;
  ...
  const otherContactsProgress = getGoogleOtherContactsSyncProgress(database, accountEmail);
  ...
  return {
    contacts:
      ...
      (otherContactsProgress
        ? countRows(
            database,
            'google_sync_other_contacts',
            accountEmail,
            otherContactsProgress.syncId
          )
        : countRows(database, 'other_contacts', accountEmail)),
...
export function getGoogleSyncState(
  ...
  const account = database
    .prepare(`SELECT contacts_sync_token, other_contacts_sync_token, calendar_list_sync_token
		FROM accounts WHERE email = ?`)
    .get(accountEmail) as Record<string, unknown> | null;
  if (!account) throw new Error(`No account is configured for ${accountEmail}.`);
  const calendars = database
    .prepare('SELECT calendar_id, sync_token FROM calendars WHERE account_email = ?')
    .all(accountEmail) as Array<Record<string, unknown>>;
  return {
    contactsSyncToken:
      account.contacts_sync_token === null ? null : String(account.contacts_sync_token),
    otherContactsSyncToken:
      account.other_contacts_sync_token === null ? null : String(account.other_contacts_sync_token),
    calendarListSyncToken:
      account.calendar_list_sync_token === null ? null : String(account.calendar_list_sync_token),
    calendarEventSyncTokens: new Map(
      calendars.map((row) => [
        String(row.calendar_id),
        row.sync_token === null ? null : String(row.sync_token),
      ])
    ),
  };
}
...
export function resetGoogleSyncState(database: DatabaseSync, accountEmail: string): void {
  withTransaction(database, () => {
    ...
    database
      ...
      .run(accountEmail);
    database
      .prepare('DELETE FROM google_other_contacts_sync_progress WHERE account_email = ?')
      .run(accountEmail);
    database
      .prepare('DELETE FROM google_sync_calendar_events WHERE account_email = ?')
      .run(accountEmail);
    ...
    database
      .prepare(`UPDATE accounts SET contacts_sync_token = NULL, other_contacts_sync_token = NULL,
			calendar_list_sync_token = NULL,
			updated_at = ? WHERE email = ?`)
      .run(new Date().toISOString(), accountEmail);
    database
      .prepare('UPDATE calendars SET sync_token = NULL WHERE account_email = ?')
      .run(accountEmail);
  });
}
...
export function applyContactsIncrementalSync(
  ...
  withTransaction(database, () => {
    ...
    const remove = database.prepare(
      'DELETE FROM contacts WHERE account_email = ? AND resource_name = ?'
    );
    for (const resourceName of deletedResourceNames) remove.run(accountEmail, resourceName);
    database
      .prepare(`UPDATE accounts SET contacts_sync_token = ?, contacts_synced_at = ?, updated_at = ?
			WHERE email = ?`)
      .run(nextSyncToken, syncedAt, syncedAt, accountEmail);
  });
}
...
export function applyCalendarListIncrementalSync(
  ...
  withTransaction(database, () => {
    ...
    database
      .prepare(`UPDATE accounts SET calendar_list_sync_token = ?, calendar_synced_at = ?, updated_at = ?
			WHERE email = ?`)
      .run(nextSyncToken, syncedAt, syncedAt, accountEmail);
  });
}
...
export function applyCalendarEventsIncrementalSync(
  ...
  withTransaction(database, () => {
    ...
    database
      .prepare(`UPDATE calendars SET sync_token = ?, updated_at = ?
			WHERE account_email = ? AND calendar_id = ?`)
      .run(nextSyncToken, syncedAt, accountEmail, calendarId);
    database
      .prepare('UPDATE accounts SET calendar_synced_at = ?, updated_at = ? WHERE email = ?')
...
export function saveContactsSyncPage(
  ...
  withTransaction(database, () => {
    ...
    if (nextPageToken) {
      database
        .prepare(
          'UPDATE google_sync_progress SET page_token = ?, updated_at = ? WHERE account_email = ? AND sync_type = ?'
        )
    ...
    database
      .prepare(
        'UPDATE google_sync_progress SET phase = ?, page_token = NULL, next_sync_token = ?, updated_at = ? WHERE account_email = ? AND sync_type = ?'
      )
...
export function saveGoogleOtherContactsSyncPage(
  database: DatabaseSync,
  progress: GoogleOtherContactsSyncProgress,
  contacts: Omit<SyncedContact, 'accountEmail'>[],
  nextPageToken: string | undefined,
  nextSyncToken?: string
): boolean {
  ...
  withTransaction(database, () => {
    ...
    if (nextPageToken) {
      database
        .prepare(`UPDATE google_other_contacts_sync_progress SET page_token = ?, updated_at = ?
				WHERE account_email = ?`)
        .run(nextPageToken, updatedAt, progress.accountEmail);
      return;
    }
    if (!nextSyncToken)
      throw new Error('Google Other Contacts full sync did not return a sync token.');
    database
      .prepare(`UPDATE google_other_contacts_sync_progress
			SET phase = 'complete', page_token = NULL, next_sync_token = ?, updated_at = ?
			WHERE account_email = ?`)
      .run(nextSyncToken, updatedAt, progress.accountEmail);
    complete = true;
  });
  return complete;
}

export function applyGoogleOtherContactsIncrementalSync(
  database: DatabaseSync,
  accountEmail: string,
  contacts: Omit<SyncedContact, 'accountEmail'>[],
  deletedResourceNames: string[],
  ...
  withTransaction(database, () => {
    ...
    const remove = database.prepare(
      'DELETE FROM other_contacts WHERE account_email = ? AND resource_name = ?'
    );
    for (const resourceName of deletedResourceNames) remove.run(accountEmail, resourceName);
    database
      .prepare(`UPDATE accounts SET other_contacts_sync_token = ?, contacts_synced_at = ?, updated_at = ?
			WHERE email = ?`)
      .run(nextSyncToken, syncedAt, syncedAt, accountEmail);
  });
}

function finishCalendarSync(
  database: DatabaseSync,
  progress: GoogleSyncProgress,
  updatedAt: string
): void {
  database
    .prepare(
      'UPDATE google_sync_progress SET phase = ?, page_token = NULL, calendar_id = NULL, updated_at = ? WHERE account_email = ? AND sync_type = ?'
    )
...
export function finalizeGoogleSync(database: DatabaseSync, accountEmail: string): void {
  ...
  const otherContacts = getGoogleOtherContactsSyncProgress(database, accountEmail);
  const calendar = getGoogleSyncProgress(database, accountEmail, 'calendar');
  if (
    !contacts ||
    contacts.phase !== 'complete' ||
    !otherContacts ||
    otherContacts.phase !== 'complete' ||
    !calendar ||
    calendar.phase !== 'complete'
  ) {
    throw new Error(
      'Google sync cannot be finalized before Contacts, Other contacts, and Calendar downloads are complete.'
    );
  }
  const syncedAt = new Date().toISOString();
  withTransaction(database, () => {
    ...
    database
      .prepare(`INSERT INTO calendars
			...
			FROM google_sync_calendars WHERE account_email = ? AND sync_id = ?`)
      .run(syncedAt, accountEmail, calendar.syncId);
    database
      .prepare(`INSERT INTO calendar_events
			...
			FROM google_sync_calendar_events WHERE account_email = ? AND sync_id = ?`)
      .run(syncedAt, accountEmail, calendar.syncId);
    database
      .prepare(`UPDATE accounts SET contacts_synced_at = ?, calendar_synced_at = ?,
			contacts_sync_token = ?, other_contacts_sync_token = ?, calendar_list_sync_token = ?, updated_at = ? WHERE email = ?`)
      .run(
        syncedAt,
        syncedAt,
        contacts.nextSyncToken,
        otherContacts.nextSyncToken,
        calendar.nextSyncToken,
        syncedAt,
        accountEmail
      );
    database
      .prepare('DELETE FROM google_sync_contacts WHERE account_email = ? AND sync_id = ?')
      .run(accountEmail, contacts.syncId);
    database
      .prepare('DELETE FROM google_sync_other_contacts WHERE account_email = ? AND sync_id = ?')
      .run(accountEmail, otherContacts.syncId);
    database
      .prepare('DELETE FROM google_other_contacts_sync_progress WHERE account_email = ?')
      .run(accountEmail);
    database
      .prepare('DELETE FROM google_sync_calendars WHERE account_email = ? AND sync_id = ?')
      .run(accountEmail, calendar.syncId);
    database
      .prepare('DELETE FROM google_sync_calendar_events WHERE account_email = ? AND sync_id = ?')
      .run(accountEmail, calendar.syncId);
    database.prepare('DELETE FROM google_sync_progress WHERE account_email = ?').run(accountEmail);
  });
}
...
export function saveCalendarListSyncPage(
  ...
  withTransaction(database, () => {
    ...
    if (!nextSyncToken)
      throw new Error('Google Calendar list full sync did not return a sync token.');
    database
      .prepare(
        'UPDATE google_sync_progress SET next_sync_token = ?, updated_at = ? WHERE account_email = ? AND sync_type = ?'
      )
      .run(nextSyncToken, updatedAt, progress.accountEmail, 'calendar');
    const first = database
      .prepare(`SELECT calendar_id FROM google_sync_calendars
			WHERE account_email = ? AND sync_id = ? ORDER BY calendar_id LIMIT 1`)
    ...
    database
      .prepare(
        'UPDATE google_sync_progress SET phase = ?, page_token = NULL, calendar_id = ?, updated_at = ? WHERE account_email = ? AND sync_type = ?'
      )
      .run('calendarEvents', first.calendar_id, updatedAt, progress.accountEmail, 'calendar');
    nextProgress = {
      ...progress,
      phase: 'calendarEvents',
      pageToken: null,
...
export function saveCalendarEventsSyncPage(
  ...
  withTransaction(database, () => {
    ...
    if (!nextSyncToken)
      throw new Error('Google Calendar event full sync did not return a sync token.');
    database
      .prepare(
        'UPDATE google_sync_calendars SET events_loaded = 1, sync_token = ? WHERE account_email = ? AND sync_id = ? AND calendar_id = ?'
      )
    ...
    database
      .prepare(
        'UPDATE google_sync_progress SET page_token = NULL, calendar_id = ?, updated_at = ? WHERE account_email = ? AND sync_type = ?'
      )
...
export function replaceContacts(
  ...
  withTransaction(database, () => {
    ...
    database
      .prepare('UPDATE accounts SET contacts_synced_at = ?, updated_at = ? WHERE email = ?')
      .run(syncedAt, syncedAt, accountEmail);
  });
}
...
export function listContacts(database: DatabaseSync, account?: string): SyncedContact[] {
  const rows = (
    account
      ? database
          .prepare(`SELECT * FROM (
			SELECT * FROM contacts WHERE account_email = ?
			UNION ALL
			SELECT other.* FROM other_contacts other WHERE other.account_email = ?
				AND NOT EXISTS (SELECT 1 FROM contacts saved WHERE saved.account_email = other.account_email
					AND saved.resource_name = other.resource_name)
      ...
      : database
          .prepare(`SELECT * FROM (
			...
			UNION ALL
			SELECT other.* FROM other_contacts other WHERE NOT EXISTS
				(SELECT 1 FROM contacts saved WHERE saved.account_email = other.account_email
					AND saved.resource_name = other.resource_name)
		) ORDER BY display_name COLLATE NOCASE, account_email`)
          .all()
...
export function listCalendars(database: DatabaseSync, account?: string): SyncedCalendar[] {
  const rows = (
    account
      ? database
          .prepare(
            'SELECT * FROM calendars WHERE account_email = ? ORDER BY summary COLLATE NOCASE'
          )
          .all(account)
      : database
          .prepare('SELECT * FROM calendars ORDER BY summary COLLATE NOCASE, account_email')
          .all()
  ) as Array<Record<string, unknown>>;
  return rows.map((row) => ({
    accountEmail: String(row.account_email),
...
export function getCalendarEvent(
  ...
  const row = database
    .prepare(
      'SELECT * FROM calendar_events WHERE account_email = ? AND calendar_id = ? AND event_id = ?'
    )
    .get(account, calendarId, eventId);
  return row ? calendarEventFromRow(row) : null;
}

export function saveCalendarResponse(
  database: DatabaseSync,
  account: string,
  calendarId: string,
  eventId: string,
...
export function listCalendarEvents(
  ...
  const rows = (
    account
      ? database
          .prepare(
            'SELECT * FROM calendar_events WHERE account_email = ? ORDER BY start_at, summary COLLATE NOCASE'
          )
          .all(account)
      : database
          .prepare('SELECT * FROM calendar_events ORDER BY start_at, summary COLLATE NOCASE')
          .all()
  ) as Array<Record<string, unknown>>;
 ...
 */
export function listCalendarEventsBetween(
  database: DatabaseSync,
  start: string,
  end: string
): SyncedCalendarEvent[] {
...
export function getIncomingEmail(
  ...
  const row = database
    .prepare(`SELECT gmail_id, thread_id, from_address, to_addresses, subject,
		message_date, snippet, body_text, body_html, body_truncated, labels_json, headers_json
		FROM emails WHERE account_email = ? AND gmail_id = ?`)
    .get(accountEmail, gmailId) as Record<string, unknown> | undefined;
  if (!row) return null;
  return {
...
export function saveEmailExtractionError(
  ...
  database
    .prepare(`UPDATE emails SET extraction_error = ?, updated_at = ?
...
export function getEmail(database: DatabaseSync, id: number, account?: string): StoredEmail | null {
  const row = database
    .prepare(
      `SELECT * FROM emails WHERE id = ? AND deleted_at IS NULL${account ? ' AND account_email = ?' : ''}`
    )
...
export function listCategories(database: DatabaseSync): Category[] {
  return database
    .prepare('SELECT id, name, description, level FROM categories ORDER BY rowid')
    .all() as Category[];
}
...
export function saveCategory(
  ...
  if (!['important', 'useful', 'other', 'auto'].includes(input.level))
    throw new CategoryValidationError('Choose a category level.');
  ...
  if (
    categories.some(
      (category) => category.id !== input.id && category.name.toLowerCase() === name.toLowerCase()
    )
  ) {
