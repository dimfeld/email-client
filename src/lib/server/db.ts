...
function readGoogleOtherContactsSyncProgress(row: Record<string, unknown>): GoogleOtherContactsSyncProgress {
	return {
		accountEmail: String(row.account_email),
		syncId: String(row.sync_id),
		phase: String(row.phase),
...
export function getGoogleOtherContactsSyncProgress(
	...
	const row = database.prepare('SELECT * FROM google_other_contacts_sync_progress WHERE account_email = ?')
		.get(accountEmail) as Record<string, unknown> | null;
	return row ? readGoogleOtherContactsSyncProgress(row) : null;
}

export function startGoogleOtherContactsSync(database: DatabaseSync, accountEmail: string): GoogleOtherContactsSyncProgress {
	const existing = getGoogleOtherContactsSyncProgress(database, accountEmail);
	if (existing) return existing;
	const syncId = randomUUID();
	const startedAt = new Date().toISOString();
	withTransaction(database, () => {
		database.prepare('DELETE FROM google_sync_other_contacts WHERE account_email = ?').run(accountEmail);
		database.prepare(`INSERT INTO google_other_contacts_sync_progress
		(account_email, sync_id, phase, page_token, next_sync_token, started_at, updated_at)
		VALUES (?, ?, 'otherContacts', NULL, NULL, ?, ?)`).run(accountEmail, syncId, startedAt, startedAt);
	});
	return { accountEmail, syncId, phase: 'otherContacts', pageToken: null, nextSyncToken: null, startedAt };
}
	...
	return {
		contacts: (contactProgress
			...
			+ (otherContactsProgress
				? countRows(database, 'google_sync_other_contacts', accountEmail, otherContactsProgress.syncId)
...
export function getGoogleSyncState(database: DatabaseSync, accountEmail: string): {
	...
	const account = database.prepare(`SELECT contacts_sync_token, other_contacts_sync_token, calendar_list_sync_token
	...
	return {
		...
		otherContactsSyncToken: account.other_contacts_sync_token === null ? null : String(account.other_contacts_sync_token),
...
export function resetGoogleSyncState(database: DatabaseSync, accountEmail: string): void {
	withTransaction(database, () => {
		database.prepare('DELETE FROM google_sync_contacts WHERE account_email = ?').run(accountEmail);
		database.prepare('DELETE FROM google_sync_other_contacts WHERE account_email = ?').run(accountEmail);
		database.prepare('DELETE FROM google_other_contacts_sync_progress WHERE account_email = ?').run(accountEmail);
		database.prepare('DELETE FROM google_sync_calendar_events WHERE account_email = ?').run(accountEmail);
		database.prepare('DELETE FROM google_sync_calendars WHERE account_email = ?').run(accountEmail);
		database.prepare('DELETE FROM google_sync_progress WHERE account_email = ?').run(accountEmail);
		database.prepare(`UPDATE accounts SET contacts_sync_token = NULL, other_contacts_sync_token = NULL,
...
export function saveGoogleOtherContactsSyncPage(
	...
	withTransaction(database, () => {
		...
		if (nextPageToken) {
			database.prepare(`UPDATE google_other_contacts_sync_progress SET page_token = ?, updated_at = ?
				WHERE account_email = ?`).run(nextPageToken, updatedAt, progress.accountEmail);
			return;
		}
		if (!nextSyncToken) throw new Error('Google Other Contacts full sync did not return a sync token.');
		database.prepare(`UPDATE google_other_contacts_sync_progress
			SET phase = 'complete', page_token = NULL, next_sync_token = ?, updated_at = ?
			WHERE account_email = ?`).run(nextSyncToken, updatedAt, progress.accountEmail);
		complete = true;
	});
	return complete;
}
...
export function applyGoogleOtherContactsIncrementalSync(
	...
	withTransaction(database, () => {
		...
		for (const contact of contacts) upsert.run(accountEmail, contact.resourceName, contact.displayName,
			JSON.stringify(contact.emails), JSON.stringify(contact.phones), contact.organization, syncedAt);
		const remove = database.prepare('DELETE FROM other_contacts WHERE account_email = ? AND resource_name = ?');
		for (const resourceName of deletedResourceNames) remove.run(accountEmail, resourceName);
		database.prepare(`UPDATE accounts SET other_contacts_sync_token = ?, contacts_synced_at = ?, updated_at = ?
			WHERE email = ?`).run(nextSyncToken, syncedAt, syncedAt, accountEmail);
	});
}
...
export function finalizeGoogleSync(database: DatabaseSync, accountEmail: string): void {
	...
	if (!contacts || contacts.phase !== 'complete' || !otherContacts || otherContacts.phase !== 'complete'
		|| !calendar || calendar.phase !== 'complete') {
		throw new Error('Google sync cannot be finalized before Contacts, Other contacts, and Calendar downloads are complete.');
	}
	const syncedAt = new Date().toISOString();
	withTransaction(database, () => {
		...
		database.prepare(`INSERT INTO other_contacts
			(account_email, resource_name, display_name, emails_json, phones_json, organization, updated_at)
			SELECT account_email, resource_name, display_name, emails_json, phones_json, organization, ?
		...
		database.prepare(`UPDATE accounts SET contacts_synced_at = ?, calendar_synced_at = ?,
			...
			.run(syncedAt, syncedAt, contacts.nextSyncToken, otherContacts.nextSyncToken,
				calendar.nextSyncToken, syncedAt, accountEmail);
		database.prepare('DELETE FROM google_sync_contacts WHERE account_email = ? AND sync_id = ?').run(accountEmail, contacts.syncId);
		database.prepare('DELETE FROM google_sync_other_contacts WHERE account_email = ? AND sync_id = ?').run(accountEmail, otherContacts.syncId);
		database.prepare('DELETE FROM google_other_contacts_sync_progress WHERE account_email = ?').run(accountEmail);
...
export function listContacts(database: DatabaseSync, account?: string): SyncedContact[] {
	const rows = (account
		...
		) ORDER BY display_name COLLATE NOCASE`).all(account, account)
		: database.prepare(`SELECT * FROM (
			...
			SELECT other.* FROM other_contacts other WHERE NOT EXISTS
				(SELECT 1 FROM contacts saved WHERE saved.account_email = other.account_email
					AND saved.resource_name = other.resource_name)
		) ORDER BY display_name COLLATE NOCASE, account_email`).all()
