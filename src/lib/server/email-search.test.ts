...
afterEach(() => { database.close(); database = createDatabase(':memory:'); });

test('indexes all fields and keeps common words in exact phrases', () => {
	upsertEmails(database, 'owner@test.com', [
		{ id: 'one', subject: 'Launch report', from: 'Alice <alice@team.example.com>', to: 'bob@test.com', bodyText: 'To be or not to be ready' },
		{ id: 'two', bodyHtml: '<p>Budget <b>approval</b> is ready</p>' }
	]);
	for (const query of ['launch', 'alice', 'bob', '"to be or not to be"', '"budget approval"']) expect(searchEmails(database, query)).toHaveLength(1);
...
test('uses BM25 rank, preserves account scope, and searches archived mail', () => {
	upsertEmails(database, 'a@test.com', [{ id: 'a', bodyText: 'kiwi kiwi kiwi' }, { id: 'b', bodyText: 'kiwi ' + 'orange '.repeat(100) }]);
...
test('matches addresses and domains exactly and accepts RFC email dates', () => {
	upsertEmails(database, 'owner@test.com', [
		{ id: 'a', from: 'Alice <alice@sub.example.com>', to: 'bob@test.com', date: 'Mon, 21 Sep 2026 12:00:00 +0000' },
		{ id: 'b', from: 'Alice <alice@notexample.com>', date: '2026-09-20T23:00:00Z' }
	]);
	expect(searchEmails(database, 'from:example.com to:bob@test.com after:2026-09-21 before:2026-09-22').map(x=>x.gmailId)).toEqual(['a']);
...
test('indexes existing messages once when an older database opens', () => {
	...
	old.exec('DROP TRIGGER email_fts_insert; DROP TRIGGER email_fts_update; DROP TRIGGER email_fts_delete; DROP TABLE email_fts');
	upsertEmails(old, 'a@test.com', [{ id: 'a', subject: 'legacy' }]); old.close();
	const migrated = createDatabase(path);
	expect(searchEmails(migrated, 'legacy')).toHaveLength(1); migrated.close();
	const reopened = createDatabase(path);
	expect(searchEmails(reopened, 'legacy')).toHaveLength(1); reopened.close();
