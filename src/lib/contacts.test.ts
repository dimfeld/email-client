...
function contact(overrides: Partial<SyncedContact>): SyncedContact {
  return {
    ...
    resourceName: `people/${overrides.displayName ?? overrides.organization ?? 'x'}`,
    displayName: '',
    emails: [],
    phones: [],
    organization: null,
...
describe('contact labels', () => {
  it('falls back from display name to organization, email, and phone', () => {
    expect(contactLabel(contact({ displayName: 'Ada Lovelace', organization: 'Analytical' }))).toBe(
      'Ada Lovelace'
    );
    expect(contactLabel(contact({ organization: 'Kaiser Lihue', phones: ['+18082465600'] }))).toBe(
      'Kaiser Lihue'
    );
  ...
  it('keys contacts by account and resource name', () => {
    expect(contactKey({ accountEmail: 'a@b.com', resourceName: 'people/1' })).toBe(
      'a@b.com\npeople/1'
    );
  });
});
