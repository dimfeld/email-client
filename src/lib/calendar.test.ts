...
function event(
  overrides: Partial<SyncedCalendarEvent> & Pick<SyncedCalendarEvent, 'startAt' | 'endAt'>
): SyncedCalendarEvent {
  return {
    accountEmail: 'owner@example.com',
    calendarId: 'primary',
    eventId: overrides.startAt,
    summary: 'Event',
    description: null,
    location: null,
    allDay: false,
...
describe('event placement', () => {
  ...
  it('sorts all-day events before timed events', () => {
    ...
    expect(eventsOnDay(events, '2026-09-21').map((item) => item.summary)).toEqual([
      'Holiday',
      'Early',
      'Late',
    ]);
  ...
  it('places overlapping timed events in side-by-side columns', () => {
    ...
    const layout = layoutTimedEvents(events, '2026-09-21').map(
      ({ event, startMinutes, endMinutes, column, columns }) => [
        event.summary,
        startMinutes,
        endMinutes,
        column,
  ...
  it('clips events that cross midnight', () => {
    ...
    expect(layoutTimedEvents(events, '2026-09-21')[0]).toMatchObject({
      startMinutes: 1320,
      endMinutes: 1440,
      continuesAfter: true,
      continuesBefore: false,
    });
    expect(layoutTimedEvents(events, '2026-09-22')[0]).toMatchObject({
      startMinutes: 0,
      endMinutes: 60,
