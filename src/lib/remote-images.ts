...
export function senderAddress(from: string): string | null {
	const address = (from.match(/<([^<>\s]+@[^<>\s]+)>/)?.[1] ?? from.match(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i)?.[0])?.toLowerCase();
...
export function allowsRemoteImages(from: string, rules: RemoteImageRule[]): boolean {
	...
	return rules.some((rule) => rule.kind === 'address' ? rule.value === address : rule.value === domain);
}
