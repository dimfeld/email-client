...
export function contactLabel(contact: SyncedContact): string {
	return contact.displayName || contact.organization || contact.emails[0] || contact.phones[0] || 'Unnamed contact';
}
...
export function contactInitials(contact: SyncedContact): string {
	const words = (contact.displayName || contact.organization || '').trim().split(/\s+/).filter(Boolean);
	if (words.length === 0) return '?';
	const initials = words.length === 1 ? words[0].slice(0, 1) : words[0].slice(0, 1) + words[words.length - 1].slice(0, 1);
	return initials.toUpperCase();
}

function groupLetter(label: string): string {
	const first = label.trim().normalize('NFD').replace(/[̀-ͯ]/g, '').slice(0, 1).toUpperCase();
...
export function contactMatches(contact: SyncedContact, query: string): boolean {
	...
	return [contact.displayName, contact.organization, ...contact.emails, ...contact.phones]
...
export function groupContacts(contacts: SyncedContact[]): ContactGroup[] {
	...
	return [...groups.entries()]
		.sort(([a], [b]) => (a === otherGroupLetter ? 1 : b === otherGroupLetter ? -1 : a.localeCompare(b)))
