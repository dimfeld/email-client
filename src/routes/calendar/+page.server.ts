...
export const actions: Actions = {
  respond: async ({ request }) => {
    ...
    if (!account?.refreshToken)
      return fail(400, { error: 'Reconnect the account in Settings before replying.' });
    if (fields.get('confirmed') !== 'yes')
      return fail(400, { error: 'Review and confirm the calendar response.' });
    try {
      const responseStatus = await respondToCalendarInvite(
        database,
        account,
        String(fields.get('calendar') ?? ''),
        String(fields.get('event') ?? ''),
        String(fields.get('response') ?? '')
      );
      return { message: 'Calendar response sent.', responseStatus };
    } catch (error) {
      return fail(502, {
        error: error instanceof Error ? error.message : 'Calendar response failed.',
      });
    }
  },
};
