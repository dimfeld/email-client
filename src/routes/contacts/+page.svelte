<script lang="ts">
  ...
  let selectedAccount = $derived(
    data.accounts.find((account) => account.email === data.selectedAccount)
  );
  let selected = $derived<SyncedContact | null>(
    visibleContacts.find((contact) => contactKey(contact) === selectedKey) ??
      groups[0]?.contacts[0] ??
      null
  );
      ...
      <div class="list-tools">
        ...
        <form method="GET">
          ...
          >
            ...
            {#each data.accounts as account (account.email)}<option
                value={account.email}
                selected={data.selectedAccount === account.email}>{account.email}</option
              >{/each}
          </select>
        </form>
      </div>
        ...
        {#each groups as group (group.letter)}
          ...
          {#each group.contacts as contact (contactKey(contact))}
            ...
            >
              ...
              <span class="row-text">
                <span class="name" class:muted={contactIsUnnamed(contact)}
                  >{contactLabel(contact)}</span
                >
                {#if contact.displayName && contact.organization}<span class="sub"
                    >{contact.organization}</span
                  >
                {:else if contact.emails[0] && contactLabel(contact) !== contact.emails[0]}<span
                    class="sub">{contact.emails[0]}</span
                  >{/if}
              </span>
            </button>
          {/each}
          ...
          <div class="empty">
            {#if data.contacts.length === 0}
              {#if selectedAccount?.contactsSyncedAt}
                ...
                <p>
                  No contacts are saved for {selectedAccount.email}. The last sync completed {new Date(
                    selectedAccount.contactsSyncedAt
                  ).toLocaleString()}. Check this account's Google Contacts, then select
                  <strong>Sync now</strong> in Settings.
                </p>
              {:else}<h2>No synced contacts</h2>
                <p>Run <code>bun run sync:google</code> to download the address book.</p>{/if}
            {:else}<h2>No matches</h2>
              <p>No contact matches “{query.trim()}”.</p>{/if}
          </div>
        {/each}
      </div>
    </div>
      ...
      {#if selected}
        <article class="detail">
          ...
          <div class="identity">
            ...
            {#if selected.displayName && selected.organization}<p class="org">
          ...
          {#if selected.emails.length}
            <section class="fields">
              <h2>Email</h2>
              {#each selected.emails as email (email)}<div class="field">
                  <a
                    href={`mailto:${email}`}
                    onclick={(event) => {
                      event.preventDefault();
                      openComposer({ mode: 'new', account: selected!.accountEmail, to: email });
                    }}>{email}</a
                  >
                </div>{/each}
            </section>
          ...
          {#if selected.phones.length}
            <section class="fields">
              <h2>Phone</h2>
              {#each selected.phones as phone (phone)}<div class="field">
        ...
        <div class="placeholder">
          <p>
            {data.contacts.length === 0
              ? 'Sync an account to see contacts here.'
