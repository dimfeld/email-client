...
<main>
	...
	<section aria-labelledby="remote-images-heading">
		...
		<p class="help">Images from these senders load when you open a message. Add a sender from the arrow beside Load remote images in a message.</p>
		<div class="sync-list">
			{#each data.remoteImageRules as rule (rule.kind + rule.value)}
				<form method="POST" action="?/removeRemoteImageRule" use:enhance class="sync-card">
					...
					<div><strong>{rule.value}</strong><p>{rule.kind === 'address' ? 'Email address' : 'Domain'}</p></div>
